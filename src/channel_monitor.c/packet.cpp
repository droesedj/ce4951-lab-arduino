#include "packet.h"

/*
   LAB WEEK 9: Data header + CRC
   CE4951-012
   @author Dennis Droese
   @filename packet.cpp
   @date 10/28/2018
*/

uint8_t crc8ccitt(const void * data, uint8_t len);

Packet::Packet(byte* data, int len, byte source, byte dest) {
  mSource = source;
  mDestination = dest;
  mLength = len;

  f_CRC = 0x01;

  // initialize mData to value of data
  for (int i = 0; i < mLength; i++) {
    mData[i] = data[i];
  }

  if (f_CRC == 0x01) {
    mCRC8 = crc8ccitt(data, mLength);
  }
  CRC8_valid = true;
}

// data is the ENTIRE rec'd packet.  len is the length of the ENTIRE packet!
Packet::Packet(byte* data, int len) {
  // MSB of rec'd packet is index 0.  The largest index is the LSB.
  // ensure the sync and version is correct.
  if (data[0] == mSynch && data[1] == mVer) {
    mSource = data[2];
    mDestination = data[3];
    mLength = data[4];
    f_CRC = data[5];

    // offset for where data begins.
    int offset = 6;
    // Loop through the data message, flip the data to be correct.
    for (int i = 0; i < mLength; i++) {
      mData[(mLength - 1) - i] = data[offset + i];
      offset++;
    }

    if (f_CRC == 0x01) {
      
      mCRC8 = data[offset];

      // Calculate the CRC8 manually.
      byte test_CRC8 = crc8ccitt(mData, mLength);
      if(mCRC8 == test_CRC8){
        CRC8_valid = true;
      } else {
        CRC8_valid = false;
      }
    } else {
      CRC8_valid = true;
    }
  }
}


bool Packet::Transmit(Transmitter trans) {
  // Sync signal
  if (!trans.transmit(mSynch, sizeof(byte))) {
    Serial.println("failed during synch");
    return false;
  }
  // Version
  if (!trans.transmit(mVer, sizeof(byte))) {
    Serial.println("failed during version");
    return false;
  }
  // Source
  if (!trans.transmit(mSource, sizeof(byte))) {
    Serial.println("failed during source");
    return false;
  }
  // Destination
  if (!trans.transmit(mDestination, sizeof(byte))) {
    Serial.println("failed during dest");
    return false;
  }
  // CRC flag
  if (!trans.transmit(f_CRC, sizeof(byte))) {
    Serial.println("failed during f_crc");
    return false;
  }
  // Length
  if (!trans.transmit(mLength, sizeof(byte))) {
    Serial.println("failed during length");
    return false;
  }
  // Data
  if (!trans.transmit(mData, mLength)) {
    Serial.println("failed during data");
    return false;
  }
  // CRC8
  if (f_CRC == 0x01) {
    if (!trans.transmit(mCRC8, sizeof(byte))) {
      Serial.println("failed during crc8");
      return false;
    }
  }

  // Everything transmitted correctly.
  return true;
}

String Packet::GetDataString() {
  String retVal;

  for (int i = 0; i < mLength; i++) {
    char c = mData[i];
    retVal.concat(c);
  }

  return retVal;
}

String Packet::GetSummary() {
  String retVal;
  retVal.concat("Sync:\t");
  retVal.concat(mSynch);
  retVal.concat("\nVer :\t");
  retVal.concat(mVer);
  retVal.concat("\nSrc :\t");
  retVal.concat(mSource);
  retVal.concat("\nDest:\t");
  retVal.concat(mDestination);
  retVal.concat("\nLen :\t");
  retVal.concat(mLength);
  retVal.concat("\nfCRC:\t");
  retVal.concat(f_CRC);
  retVal.concat("\nData:\t");
  retVal.concat(GetDataString());
  retVal.concat("\nCRC8:\t");
  if(f_CRC == 0x01){
    retVal.concat(mCRC8);
  } else {
    retVal.concat("N/A");
  }

  return retVal;
}

byte Packet::GetSource(){
  return mSource;
}

byte Packet::GetDestination(){
  return mDestination;
}

/*
  Public domain CRC8-CCITT implementation
*/
static const uint8_t CRC_TABLE[256] = {
  
};

uint8_t crc8ccitt(const void * data, uint8_t len) {
  uint8_t val = 0;

  uint8_t * Pos = (uint8_t *) data;
  uint8_t * End = Pos + len;

  while (Pos < End) {
    val = CRC_TABLE[val ^ *Pos];
    Pos++;
  }

  return val;
}
